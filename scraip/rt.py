...
def rtget():
    ...
    # 掲載順位や順位を入れるリスト
        ...
        def access_get(self):
            for num, url in zip(self.__yados_num, self.__urls):
    ...
    # ここから掲載順位を見つける為の関数   
        ...
        def movement_area(self):
            ...
            if area == "東京２３区内" or area == "大阪" or area == "京都":
                ...
                all_yado_nums = driver.find_element_by_xpath('//*[@id="srchMainContent"]/div[2]/p/span/em').text
                self.__rt_reviews_dic["全体件数"] = all_yado_nums
            else:
                driver.find_element_by_link_text(self.__rt_reviews_dic["エリア"]).click()
    ...
    # kyosen.movement_area()
    # kyosen.repeat_next_page()
    # kyosen.yado_rank()
    # kyosen.preservation()
